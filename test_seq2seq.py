...
df = prepare_data(df_raw).dropna(axis=0, how="any")
...
print(df_test.info()) # 318

X_train = df_train.loc[:, ['PM10', 'RR', 'TN', 'TX', 'TM', 'PMERM', 'FFM', 'DXY', 'UM', 'GLOT']].values.copy()
X_test = df_test.loc[:, ['PM10', 'RR', 'TN', 'TX', 'TM', 'PMERM', 'FFM', 'DXY', 'UM', 'GLOT']].values.copy()
y_train = df_train['PM10'].values.copy().reshape(-1, 1)
y_test = df_test['PM10'].values.copy().reshape(-1, 1)
